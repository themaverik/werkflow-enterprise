...
import { listCustodyMappings, createCustodyMapping, updateCustodyMapping, deleteCustodyMapping, type CustodyMappingResponse, type CustodyMappingRequest } from '@/lib/api/custody'
...
export default function CustodyMappingsPage() {
  ...
  const { data: mappings, isLoading, error, refetch } = useQuery({
    queryKey: ['custody-mappings', TENANT_CODE],
    queryFn: () => listCustodyMappings(TENANT_CODE),
  ...
  const validateForm = (): boolean => {
    if (!formState.categoryKey.trim()) {
      setFormError('Category Key is required')
      return false
    }
    if (!formState.custodyGroup.trim()) {
      setFormError('Custody Group is required')
      return false
    }
    return true
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const request: CustodyMappingRequest = {
        ...
        categoryKey: formState.categoryKey.trim(),
        custodyGroup: formState.custodyGroup.trim(),
      ...
      setEditingId(null)
      queryClient.invalidateQueries({ queryKey: ['custody-mappings', TENANT_CODE] })
    } catch (err) {
  ...
  const handleEdit = (mapping: CustodyMappingResponse) => {
    setFormState({
      ...
      custodyGroup: mapping.custodyGroup,
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="categoryKey">{t('categoryKey')}</Label>
                  <Input
                    id="categoryKey"
                    placeholder={t('categoryKeyPlaceholder')}
                    value={formState.categoryKey}
                    onChange={e => handleFormChange('categoryKey', e.target.value)}
                    disabled={submitting}
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="custodyGroup">{t('custodyGroup')}</Label>
                  <Input
                    id="custodyGroup"
                    placeholder={t('custodyGroupPlaceholder')}
                    value={formState.custodyGroup}
                    onChange={e => handleFormChange('custodyGroup', e.target.value)}
                    disabled={submitting}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    id="displayName"
                    placeholder="e.g., IT Team Laptops"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-4 font-semibold">{t('categoryKey')}</th>
                    <th className="text-left py-3 px-4 font-semibold">{t('custodyGroup')}</th>
                  ...
                  {mappings.map((mapping) => (
                    <tr key={mapping.id} className="border-b hover:bg-muted/50 transition-colors">
                      <td className="py-3 px-4">
                        <code className="bg-muted px-2 py-1 rounded text-xs">{mapping.categoryKey}</code>
                      </td>
                      <td className="py-3 px-4">{mapping.custodyGroup}</td>
