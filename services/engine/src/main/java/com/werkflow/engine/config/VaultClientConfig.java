 ...
 *
 * <p>Engine-service uses the {@code werkflow-engine} OpenBao token (read-only on
 * {@code secret/data/tenants/+/+/+}) to resolve tenant credentials at delegate
 * execution time for M4.12 Phase B.2. See ADR-020 and
...
public class VaultClientConfig {
     ...
     * Engine reads are scoped read-only by the {@code werkflow-engine} token policy.
     */
    @Bean
