...
public class FlowableConfig {
    ...
    public EngineConfigurationConfigurer<SpringProcessEngineConfiguration> processEngineConfigurer(
        ...
        return engineConfiguration -> {
            ...
            // Register Werkflow parse handlers + validator family (SendTask / ScriptTask /
            // BusinessRuleTask / ManualTask) via the shared customizer, so the production engine
            // and the in-memory test engine (WerkflowTestProcessEngine) stay in sync.
            // ADR-015 / Send-Task.md §6 D-ST-1.
            WerkflowProcessEngineCustomizer.applyValidatorsAndParseHandlers(engineConfiguration);
        };
    }
}
