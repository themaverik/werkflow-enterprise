...
- **Process quality gate (ADR-028 Level 2):** `AllProcessesDeployAndStartTest` — parameterized deploy+start gate covering all 8 example BPMNs. Runs in the standard `mvn test` / `mvn verify` lifecycle (no profile needed). When authoring a new example BPMN, register its process key in `allProcessKeys()` to gate it automatically. Note: this gate covers classpath (example) BPMNs only — processes authored via the BPMN designer hit deploy-time validators (Level 1) automatically; a full start gate for UI-authored processes is tracked as a post-MVP improvement (see master roadmap → Post-MVP backlog).
