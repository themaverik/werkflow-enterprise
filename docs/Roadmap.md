...
**Last Updated**: 2026-06-22 (session 40)
...
| Next | MVP release cut after user runs manual E2E sweep (sessions 34–40 shipped; example seeding rewritten as logical unit; "No Start Form" fixed; git history rewritten via `git filter-repo`) |
| Branch | `main` — all feature branches merged + force-pushed clean history (HEAD `9b6b29f5`) |
| Operational | Dev DBs continue to use additive V13–V23 migrations. V23 (session 40) hard-deletes 4 stale system drafts (`asset-request-process`, `general-approval`, `event-ticket-request`, `onboarding-checklist`). `ProcessExampleDeployer` now self-heals on every boot — no future cleanup migrations needed for example removals. `.env.shared` must define `ENGINE_CLIENT_SECRET` + `ADMIN_CLIENT_SECRET` + `KEYCLOAK_CLIENT_SECRET` + `KEYCLOAK_ADMIN_PASSWORD` + `PGADMIN_PASSWORD` (consolidated session 40, `a6d5e880`). |
