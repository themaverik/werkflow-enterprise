...
| Next | **See master Roadmap "Pre-MVP Punch List" (logged 2026-06-01)** — 7 items: nav search removal, branding+content review, DLQ design question, dept-based custody mappings, ERP bearer-token registration flow, documentation+tutorials, designer "capabilities unavailable" race condition |
| Branch | main (session 18 pushed; session 19 7 commits NOT pushed yet — pending user confirm) |
| Operational | dev/live admin DBs need `mvn flyway:repair` (V6 + V24); V12/V13/V14/V15/V16 are additive — safe on live schemas. V15 + V16 require `WERKFLOW_RESET_EXAMPLES=true` + `WERKFLOW_DEPLOY_EXAMPLES=true` + `docker compose up -d --force-recreate engine-service` for indicator backfill + DMN seed (one-shot; revert flags after) |
