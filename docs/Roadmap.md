...
| Next | Pre-MVP hard gate (correctness over speed per user): (a) Verify `7f23b1a1` rebuild — form display names propagate to DB; push commit; (b) Address post-compaction punch list (UI inconsistencies + design system audit + card transitions + admin health proxy); (c) Manual E2E Option A — 10-test baseline run against current code; (d) **D2 form tenant scoping** (Flyway V27+ adds `tenant_id` to `form_schemas` + thread tenantId through `BpmnFormKeyPinner` / `FormSchemaService` / `FormSchemaController`, 3-5h); (e) **D1 R1 hardening + Amendments 1+5** (engine `BpmnFormKeyPinner` + `BundleDeploymentService` + standalone `/deploy` fail-loud on missing form/DMN refs + portal toast pattern, 1-3h); (f) **Seed library reshuffle** (finance-approval removal + IT Helpdesk authoring with self-contained `NotificationDelegate` verification, 3-4h); (g) Re-run authoring tests (Tests 2, 3, 6) against final code. Option B fallback (single E2E pass after all code lands) recorded but not preferred. |
| Pre-MVP UI Punch List (2026-06-22) | (1) **Datasource page UI inconsistency** — page background still grey; should use dialogs for forms (matching connectors/tenant onboarding); datasource list should render like connector list. (2) **Design system consistency audit** — verify `DESIGN.md` exists across portal; 2-3 prior iterations haven't fully unified; need single theme + tokens source of truth. (3) **Card transition/effect consistency** — Marketplace Connector cards have polished hover/transitions; other Werkflow cards lack them. Make consistent without changing card layouts (only transitions, borders, drop shadows). (4) **Admin Process Health "down" recurring proxy issue** — `http://localhost:8083` shows DOWN in /processes/health UI when admin is UP. Suspected portal-proxy env var issue. Root-cause fix instead of patching every recurrence. |
| Branch | `main` — HEAD `7f23b1a1` (engine form-name fix not yet pushed; docs + V24-V26 + XSD + validator already pushed up to `4876f17c`) |
| Operational | Dev DB at V26. Engine rebuild required after `7f23b1a1`: `cd services/engine && mvn clean package -DskipTests && cd ../../infrastructure/docker && docker compose build engine && docker compose up -d engine`. `ProcessExampleDeployer` self-heals on boot. `.env.shared` requirements unchanged from session 40. Docker compose env warnings for `${ENGINE_CLIENT_SECRET}`/`${ADMIN_CLIENT_SECRET}`/etc. are cosmetic — `env_file:` directives populate containers correctly; warnings can be silenced via `--env-file ../../config/env/.env.shared` flag or `.env` symlink in `infrastructure/docker/`. |
