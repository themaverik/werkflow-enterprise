...
| Branch | `main` — HEAD `aa5e503e` (s51: Role Mappings merged + credential-test SpEL fix + pre-MVP MUST scope) |
...
- **Session 51** (2026-07-02, `e3b27b19`→`aa5e503e`, pushed) — Role Mappings Tier-2 fix merged; **credential-test 500 fixed** (`f98960ab`): Spring `@Cacheable` unwraps `Optional`, so `#result` in `unless` was the unwrapped DTO and `!#result.isPresent()` threw `SpelEvaluationException` on any non-empty resolve → `unless = "#result == null"` + real-cache-proxy regression test. **ERP connector now authenticates end-to-end** via a manual HTTP-header-auth credential (`X-API-Key` = plaintext key; ERP stores only the SHA-256 hash) — verified. Known issues logged: connector one-click "Generate & Register ERP Key" stores `Authorization` not `X-API-Key` (this repo); ERP `AccessDeniedException`→500-not-403 (erp `04a3577`). Added pre-MVP MUST scope (business integrations Google Cal+Gmail/MCP/Slack/WhatsApp/EventBridge + marketplace one-click auto-provision). Resilience confirmed: per-request `@RestControllerAdvice` contains handler 500s (all 3 services health 200; boot-time fail-fast is the only intentional crash).
